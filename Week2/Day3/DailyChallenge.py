...
# Goal: Create a dictionary that stores the indices (number of the position) of each letter 
...
for i, item in enumerate(user_word):
    if item not in letter_position:
        letter_position[item] = [i]
    else:
        letter_position[item].append(i)
...
# Instructions:
# 1. Store Data:
# You will be provided with a dictionary (items_purchase) where the keys are the item names and 
# the values are their prices (as strings with a dollar sign). The priority is defined by the position 
# of the iten on the dictionary: from the most important to the less important.
...
basket = [] #will be written items, that can be afforded

wallet_num = int(wallet.replace('$', ''))

for item, value in items_purchase.items():
    clean_value=int(value.replace ('$', '').replace (',', ''))
    if clean_value <=wallet_num:
        basket.append(item)
        wallet_num -= clean_value
